...
class AuthController extends Notifier<AuthState> {
    ...
    result.fold(
      onSuccess: (user) async {
        if (user != null) {
          ...
          final expiresAt = await ref.read(secureStorageProvider).readTokenExpiry();
  ...
  void _scheduleRefresh(DateTime expiresAt) {
    ...
    final delay = expiresAt.subtract(_refreshLeadTime).difference(DateTime.now());
    _refreshTimer = Timer(delay.isNegative ? Duration.zero : delay, _performScheduledRefresh);
  }
...
class ForgotVerifyController extends Notifier<AuthFormState> {
  ...
  Future<Result<void>> verify({required String contact, required String code}) async {
    state = state.copyWith(status: FormStatus.submitting, clearError: true);
